			...
			<div class="sarticleLeft L">
				<div class="sarticleLeftInner">
					...
					<div class="detailPage">
						<h3>{{$zixun->title}}</h3>
						<p><span>文章来源：{{$zixun->news_url}}</span>
							<span>作者:{{$zixun->drive}}</span>
							<span>浏览量：{{$zixun->look_num}}</span>
							<span class="noMargin">发表时间：{{date('Y-m-d',$zixun->created_at)}}</span>
						</p>
						<div>
							 {!! html_entity_decode($zixun->editorValue) !!}
						</div>
		            </div>
		            <!-- 阅读推荐 -->
		            <div class="recommend">
			            	...
			            	<div class="recommendContent">
			            		<div class="L">
			            			<ul>
			            				...
			            				<li>
			            					<em></em><a href="{{url('/zx_details')}}?id={{$tuijian->id}}">{{$tuijian->title}}</a>
			            				</li>
			            				@endforeach
			            			</ul>
			            		</div>
